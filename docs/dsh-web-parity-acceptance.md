...
The native history transport now applies a `4,096` raw-event safety bound after DSH message-boundary selection. This keeps a single very large streamed message from making the initial JSON payload and virtual Trajectory mount unbounded; older rows remain addressable through the existing `beforeSeq` pagination contract. On the real Super Mario session `s-f15a7e57` (`105,982` events at attach), Chromium completed the DSH search/session-selection path and mounted the Trajectory table. A `25`-second observation recorded prompt admission `9ms`, `3` mux frames (`105,980→105,982`), event→UI samples `1,539–1,581ms`, minimum/average FPS `60/60.8`, DOM peak `657`, heap `28→36MiB`, and `3` Long Tasks totaling `419ms`. The run also exposed two existing DSH `assistant-step` `invalid turn -1` console errors and the model turn entered pressure compaction, so this evidence does not close the continuous reasoning/token/tool stream or zero-error performance gates.
...
The current `deepseek-harness/apps/web` checkout contains source and dependencies but no built DSH `dist` or assembled `dsh web` boot manifest; its Vite configuration intentionally rejects standalone serving. It therefore cannot provide an independent reference capture without building/running the read-only source checkout, so the committed native screenshots and geometry JSON remain evidence rather than a claimed pixel diff. The local Playwright installation has Chromium only; Firefox, WebKit, NVDA, and JAWS executables are absent. WSL2 is available with AlmaLinux-8 and has already passed the Linux native startup/API/WebSocket and local upgrade/rollback smoke, but it does not supply the missing reference UI or screen-reader engines. These are environment prerequisites for the remaining P36-6.2/P36-6.3 and formal P36-8.2–P36-8.4 acceptance items.
