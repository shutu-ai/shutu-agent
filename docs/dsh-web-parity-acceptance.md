...
The same fixture now includes an assistant text block alongside reasoning and tool calls. A Playwright run also clicked a search result, opened the request details panel, toggled the native Turns collapse control and expanded it again, then loaded an earlier history page. The run completed with `eventCount: 10000`, `trajectoryRows: 33`, logical row count `34`, `domNodes: 887`, `scrollHeight: 1141`, `61` sampled frames after settling, and zero browser errors. This validates the repeatable P36-5.3 interaction path; tool-call collapse remains dependent on a complete upstream assistant/tool grouping in the loaded page, while production scroll-anchor and continuous-growth acceptance remain open.
