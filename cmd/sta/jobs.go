...
func (a *app) onJobSettled(snap jobs.JobSnapshot, output string) {
	...
	metadata := jobCompletionMetadata(snap.ID, string(snap.Kind), snap.Label, string(snap.Status))
...
// jobCompletionMetadata preserves the DSH tool-jobs notice contract on the
// durable inbox message. Web projects this source as a compact completion
// notice instead of an ordinary user turn.
func jobCompletionMetadata(id, kind, label, status string) map[string]string {
