...
// TestNativeCLICrossEntryFixture drives the production sta binary through one
...
func TestNativeCLICrossEntryFixture(t *testing.T) {
	...
	configPath := filepath.Join(root, "sta.yaml")
...
`
	...
	binary := filepath.Join(root, "sta-under-test.exe")
	build := exec.Command("go", "build", "-o", binary, "./cmd/sta")
	build.Dir = "../.."
