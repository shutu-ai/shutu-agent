...
func TestMinimalRegistersOnlyPersistentPlatformShell(t *testing.T) {
	...
	// Windows can take several seconds to materialize a fresh persistent pwsh
	// under parallel test load; the shared terminal smoke timeout is otherwise
	// too short for this real-subprocess test.
	app.cfg.Terminal.ReadTimeoutMS = 15000
	if err := app.registerTerminal(); err != nil {
		t.Fatalf("registerTerminal: %v", err)
	}
	want := "bash"
