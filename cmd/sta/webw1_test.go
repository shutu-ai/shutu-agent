...
func TestWebMessageRichPromptIsOneDurableSourceAttributedMessage(t *testing.T) {
	...
	userEvents := make([]session.Event, 0)
	for _, event := range a.log.Events() {
		if event.Type == session.EventUserMessage && bytes.Contains(event.Data, []byte(`"type":"image"`)) {
			userEvents = append(userEvents, event)
		}
	}
	if len(userEvents) != 1 {
		t.Fatalf("rich user message events = %d of %d, want one durable rich prompt", len(userEvents), len(a.log.Events()))
	}
	var payload struct {
		Content []struct {
	...
	if err := json.Unmarshal(userEvents[0].Data, &payload); err != nil {
		t.Fatal(err)
	}
	if len(payload.Content) != 2 || payload.Content[0].Type != "image" ||
		payload.Content[1].Type != "text" || payload.Content[1].Text != "describe this" {
		t.Fatalf("rich prompt content = %+v", payload.Content)
	}
	if payload.Source == nil || payload.Source.Kind != "user" || payload.Source.RPCID != "rich-rpc" ||
		payload.Source.ClientTimeZone != "Asia/Shanghai" {
		t.Fatalf("rich prompt source = %+v", payload.Source)
	}
}
...
func TestWebMessageSurvivesRequestDisconnect(t *testing.T) {
	...
	go func() { errCh <- a.webMessage(requestCtx, "s-a", "continue", nil, webserver.PromptMeta{}) }()
...
func TestNativeCreateAgentSessionAdoptsNamedIDWithDSHConflicts(t *testing.T) {
	st, err := store.OpenSQLite(filepath.Join(t.TempDir(), "native-create.db"))
	if err != nil {
		t.Fatal(err)
	}
	defer st.Close()
	...
	if err := st.CreateSession(ctx, "named-existing", time.Now()); err != nil {
		t.Fatal(err)
	}
	if err := st.SetSessionCWD(ctx, "named-existing", existingCWD); err != nil {
		t.Fatal(err)
	}
	if err := st.SetSessionConfig(ctx, "named-existing", store.SessionConfig{AgentPreset: "code"}); err != nil {
		t.Fatal(err)
	}
	a := &app{
		store:         st,
	...
	result, err := a.nativeCreateAgentSession(ctx, webserver.NativeSessionCreateSpec{
		SessionID: "named-existing", CWD: existingCWD, AgentPreset: "code", AgentPresetRequested: true,
	})
	if err != nil || result.SessionID != "named-existing" || result.AgentPreset != "code" {
		t.Fatalf("adopt existing result=%#v err=%v", result, err)
	}
	result, err = a.nativeCreateAgentSession(ctx, webserver.NativeSessionCreateSpec{
		SessionID: "named-existing", CWD: existingCWD, AgentPreset: "minimal",
	})
	if err != nil || result.SessionID != "named-existing" || result.AgentPreset != "code" {
		t.Fatalf("adopt existing with different default result=%#v err=%v", result, err)
	}
	_, err = a.nativeCreateAgentSession(ctx, webserver.NativeSessionCreateSpec{
		SessionID: "named-existing", CWD: existingCWD, AgentPreset: "minimal", AgentPresetRequested: true,
	})
	var createErr *webserver.NativeSessionCreateError
	if !errors.As(err, &createErr) || createErr.Code != "agent-preset-conflict" ||
		createErr.Details["existingPreset"] != "code" || createErr.Details["requestedPreset"] != "minimal" {
		t.Fatalf("preset conflict = %#v, err=%v", createErr, err)
	}

	newCWD := filepath.Join(t.TempDir(), "projects", "shutu")
	result, err = a.nativeCreateAgentSession(ctx, webserver.NativeSessionCreateSpec{
		SessionID: "named-new", CWD: newCWD, AgentPreset: "minimal", AgentPresetRequested: true,
	})
	if err != nil || result.SessionID != "named-new" || result.AgentPreset != "minimal" {
		t.Fatalf("create named result=%#v err=%v", result, err)
	}
	meta, err := st.GetSessionMeta(ctx, result.SessionID)
	if err != nil || meta.CWD != newCWD {
		t.Fatalf("named create meta=%#v err=%v", meta, err)
	}
	info, err := os.Stat(newCWD)
	if err != nil || !info.IsDir() {
		t.Fatalf("fresh create cwd=%q stat=%#v err=%v", newCWD, info, err)
	}
	config, err := st.GetSessionConfig(ctx, result.SessionID)
	if err != nil || config.AgentPreset != "minimal" {
		t.Fatalf("named create config=%#v err=%v", config, err)
	}
}
...
func TestWebSessionNewThenMessageAfterRequestCtxCancelled(t *testing.T) {
	...
	if err := a.webMessage(context.Background(), sid, "hello", nil, webserver.PromptMeta{}); err != nil {
