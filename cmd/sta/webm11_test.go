...
func TestNativeSettingsApplyRebuildsProviderRuntime(t *testing.T) {
	a, st := m11App(t)
	ctx := context.Background()
	t.Setenv("OLLAMA_API_KEY", "ollama-key")

	err := a.applyNativeProviderSettingsRuntime(ctx, "llm-deepseek", map[string]any{
		"apiKeyEnv": "DEEPSEEK_API_KEY",
		"baseURL":   "https://deepseek-override.example/v1",
		"model":     "deepseek-v4-pro",
		"models": []any{
			map[string]any{"id": "deepseek-v4-pro", "name": "DeepSeek V4 Pro", "contextWindow": 128000, "maxTokens": 8192},
		},
	})
	if err != nil {
		t.Fatalf("apply deepseek settings: %v", err)
	}
	providers := a.webProviders()
	deepseek := findProvider(providers, "deepseek-official")
	if deepseek == nil || deepseek["base_url"] != "https://deepseek-override.example/v1" || deepseek["model"] != "deepseek-v4-pro" {
		t.Fatalf("deepseek runtime view = %#v", deepseek)
	}
	got, err := st.GetSettings(ctx)
	if err != nil || got["llm.profile.deepseek-official"] == "" {
		t.Fatalf("deepseek profile persisted = %#v, err=%v", got, err)
	}

	err = a.applyNativeProviderSettingsRuntime(ctx, "llm-pi-ai", map[string]any{
		"providers": map[string]any{
			"openai": map[string]any{
				"api": "openai-completions", "apiKeyEnv": "OPENAI_API_KEY",
				"baseURL": "https://openai-override.example/v1", "model": "gpt-4.1-mini",
			},
			"ollama": map[string]any{
				"api": "openai-completions", "apiKeyEnv": "OLLAMA_API_KEY", "displayName": "Ollama",
				"baseURL": "http://127.0.0.1:11434/v1", "model": "llama3.1",
			},
		},
	})
	if err != nil {
		t.Fatalf("apply pi-ai settings: %v", err)
	}
	if _, err := a.llmReg.Get("ollama"); err != nil {
		t.Fatalf("custom provider not registered: %v", err)
	}
	openai := findProvider(a.webProviders(), "openai")
	if openai == nil || openai["base_url"] != "https://openai-override.example/v1" || openai["model"] != "gpt-4.1-mini" {
		t.Fatalf("openai runtime view = %#v", openai)
	}
	got, err = st.GetSettings(ctx)
	if err != nil || got["llm.custom.ollama"] == "" || got["llm.profile.openai"] == "" {
		t.Fatalf("pi-ai provider settings persisted = %#v, err=%v", got, err)
	}
	...
	if err != nil {
		t.Fatalf("apply pi-ai removal: %v", err)
	}
	if _, err := a.llmReg.Get("ollama"); err == nil {
		t.Fatal("removed custom provider remains registered")
	}
	got, err = st.GetSettings(ctx)
	if err != nil || got["llm.custom.ollama"] != "" {
		t.Fatalf("removed custom provider settings = %#v, err=%v", got, err)
	}
}

func TestNativeSettingsApplyBridgesDefaultModelAndPermission(t *testing.T) {
	a, st := m11App(t)
	ctx := context.Background()

	if err := a.applyNativeProviderSettingsRuntime(ctx, "agent-default-model", map[string]any{
		"provider": "deepseek-official", "model": "deepseek-v4-pro",
		"reasoningEffort": "high",
	}); err != nil {
		t.Fatalf("apply default model settings: %v", err)
	}
	a.providerMu.RLock()
	...
	if provider != "deepseek-official" || model != "deepseek-v4-pro" || effort != "high" {
		t.Fatalf("default model runtime = %s/%s/%s", provider, model, effort)
	}
	settings, err := st.GetSettings(ctx)
	if err != nil || !strings.Contains(settings["agent_default_model"], "deepseek-v4-pro") {
		t.Fatalf("default model persistence = %#v, err=%v", settings["agent_default_model"], err)
	}

	if err := a.applyNativeProviderSettingsRuntime(ctx, "permission", map[string]any{
		"defaultPreset": "readonly",
	}); err != nil {
		t.Fatalf("apply permission settings: %v", err)
	}
	settings, err = st.GetSettings(ctx)
	if err != nil || settings["permission_preset"] != "readonly" {
		t.Fatalf("permission persistence = %#v, err=%v", settings["permission_preset"], err)
	}
}

func TestNativeRuntimeSettingsRestoreShellAndAgentLoop(t *testing.T) {
	a, st := m11App(t)
	a.reg = tools.New()
	a.basePolicy = tools.DefaultPolicy()
	ctx := context.Background()
	if err := st.SetSetting(ctx, "native.settings.shell", `{"value":{"timeoutMs":5000,"maxTimeoutMs":10000,"maxOutputBytes":2048,"maxSpillBytes":4096,"graceMs":0,"cwd":"C:\\shell-root","pwshPath":"pwsh"}}`); err != nil {
		t.Fatal(err)
	}
	if err := st.SetSetting(ctx, "native.settings.agent-loop", `{"value":{"maxParallelToolCalls":3}}`); err != nil {
		t.Fatal(err)
	}
	if err := st.SetSetting(ctx, "native.settings.web-search-deepseek", `{"value":{"apiKeyEnv":"WEB_SEARCH_RUNTIME_KEY","baseURL":"https://search.example/anthropic/v1","model":"deepseek-v4-pro","apiVersion":"2024-01-01","maxTokens":2048,"maxUses":2}}`); err != nil {
		t.Fatal(err)
	}
	if err := a.loadNativeRuntimeSettings(ctx); err != nil {
		t.Fatal(err)
	}

	settings := a.nativeShellSettings()
	if settings.TimeoutMS != 5000 || settings.MaxTimeoutMS != 10000 || settings.MaxOutputBytes != 2048 ||
		settings.MaxSpillBytes != 4096 || settings.GraceMS != 0 || settings.Cwd != `C:\shell-root` || settings.PwshPath != "pwsh" {
		t.Fatalf("restored shell settings = %#v", settings)
	}
	policy := a.reg.Policy()
	if policy.Shell.Timeout != 10*time.Second || policy.Shell.OutputLimit != 2048 || policy.Shell.MaxSpillBytes != 4096 {
		t.Fatalf("restored shell policy = %#v", policy.Shell)
	}
	if got := a.nativeAgentLoopMaxParallel(); got != 3 {
		t.Fatalf("restored agent-loop limit = %d, want 3", got)
	}
	search := a.nativeWebSearchSettingsSnapshot()
	if search.APIKeyEnv != "WEB_SEARCH_RUNTIME_KEY" || search.BaseURL != "https://search.example/anthropic/v1" ||
		search.Model != "deepseek-v4-pro" || search.APIVersion != "2024-01-01" || search.MaxTokens != 2048 || search.MaxUses != 2 {
		t.Fatalf("restored web search settings = %#v", search)
	}
}
