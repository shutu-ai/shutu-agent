...
func TestSessionInboxJournalEncodesEmptyInsertedAsArray(t *testing.T) {
	log := session.New()
	journal := sessionInboxJournal{log: log}
	err := journal.AppendInboxEvent(agent.InboxEvent{
	...
	events := log.Events()
	if len(events) != 1 || events[0].Type != session.EventAgentInboxSpliced {
		t.Fatalf("journal events = %+v, want one inbox splice", events)
	}
	...
	if !ok || len(inserted) != 0 {
		t.Fatalf("durable inbox inserted = %#v, want []", data["inserted"])
	}
}
...
func TestRecoverSubagentCompletionWakeAfterDurableEnd(t *testing.T) {
	...
	if pending[0].Metadata["dedupe_key"] != "subagent:end:child-1" ||
		pending[0].Metadata["source_kind"] != "subagent-settled" ||
		pending[0].Metadata["source_form"] != "notice" ||
		pending[0].Metadata["source_sender_session_id"] != "child-1" ||
		!strings.HasPrefix(pending[0].Metadata["source_summary"], "Background subagent child-1 finished") {
		t.Fatalf("pending completion wake = %+v", pending[0])
	}
}
...
func TestRecoverSubagentReportRelay(t *testing.T) {
	log := session.New()
	if _, err := log.Append(session.EventSubagentReport, session.NewSubagentReportWithID("child-1", "parent-1", "selected findings", "report-1")); err != nil {
		t.Fatal(err)
	}
	registry := agent.NewRegistry()
	defer registry.CloseAll()
	...
	if err := (&app{}).recoverSubagentReportRelays(log, handle); err != nil {
		t.Fatalf("recover report relay: %v", err)
	}
	if err := (&app{}).recoverSubagentReportRelays(log, handle); err != nil {
		t.Fatalf("repeat report recovery: %v", err)
	}
	events := log.Events()
	if len(events) != 2 || events[1].Type != session.EventAgentInboxSpliced {
		t.Fatalf("report recovery events = %+v, want exactly one inbox splice", events)
	}
	pending := handle.Agent().Inbox().PendingMessages()
	if len(pending) != 1 || pending[0].Metadata["dedupe_key"] != "subagent:report:report-1" ||
		pending[0].Metadata["source_kind"] != "subagent-report" ||
		pending[0].Metadata["source_form"] != "relay" ||
		pending[0].Metadata["source_sender_session_id"] != "child-1" ||
		pending[0].Text != "Background subagent child-1 reported:\nselected findings" {
		t.Fatalf("pending report relay = %+v", pending[0])
	}
	if got := len(handle.Agent().Inbox().PendingMessages()); got != 1 {
		t.Fatalf("pending report relays = %d, want 1", got)
	}
}
...
func TestRecoverJobCompletionWakeAfterDurableDone(t *testing.T) {
	...
	if len(pending) != 1 || pending[0].Metadata["dedupe_key"] != "job:job-1" ||
		pending[0].Metadata["source_kind"] != "plugin" ||
		pending[0].Metadata["source_plugin"] != "tool-jobs" ||
		pending[0].Metadata["source_form"] != "notice" ||
		pending[0].Metadata["source_summary"] != "job-1 [completed]" {
		t.Fatalf("pending job wake = %+v", pending)
	}
}

func TestInboxSourceMetadataProjectsToDSHNotice(t *testing.T) {
	model := llm.Message{Role: llm.RoleUser, Content: []llm.ContentBlock{llm.Text("completed")}}
	applyInboxMessageSource(&model, map[string]string{
		"source_kind":    "plugin",
		"source_plugin":  "tool-jobs",
		"source_form":    "notice",
		"source_summary": "bash test [completed]",
	})
	raw, err := json.Marshal(session.NewUserMessageAt(1, 1, 1, model))
	if err != nil {
		t.Fatal(err)
	}
	var data struct {
	...
	if data.Source == nil || data.Source.Kind != "plugin" || data.Source.Plugin != "tool-jobs" ||
		...
		t.Fatalf("projected inbox source = %+v, want DSH notice", data.Source)
	}
}

func TestInboxUserRPCMetadataProjectsToDSHTimeZoneSource(t *testing.T) {
	model := llm.Message{Role: llm.RoleUser, Content: []llm.ContentBlock{llm.Text("schedule this")}}
	applyInboxMessageSource(&model, map[string]string{
		"rpc_id":           "rpc-zone",
		"client_time_zone": "Asia/Shanghai",
	})
	raw, err := json.Marshal(session.NewUserMessageAt(1, 1, 0, model))
	if err != nil {
		t.Fatal(err)
	}
	var data struct {
	...
	if data.Source == nil || data.Source.Kind != "user" || data.Source.RPCID != "rpc-zone" ||
		data.Source.ClientTimeZone != "Asia/Shanghai" {
		t.Fatalf("projected user source = %+v", data.Source)
	}
}

func TestSubagentCompletionMetadataMatchesDSH(t *testing.T) {
	metadata := subagentCompletionMetadata("child-long", "completed")
	summary := subagentSettlementSummary("child-long", "completed")
	if metadata["source_kind"] != "subagent-settled" ||
		metadata["source_form"] != "notice" ||
		metadata["source_sender_session_id"] != "child-long" ||
		metadata["source_summary"] != summary {
		t.Fatalf("completion metadata = %+v", metadata)
	}

	longChild := strings.Repeat("x", 99)
	bounded := boundSubagentSummary(subagentSettlementSummary(longChild, "unknown-reason"))
	if got := []rune(bounded); len(got) != 120 || got[119] != '…' {
		t.Fatalf("bounded summary length = %d suffix = %q, want 120 runes ending ellipsis", len(got), got[len(got)-1])
	}
}
