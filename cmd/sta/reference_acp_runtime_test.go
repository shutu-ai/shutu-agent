...
func writeReferenceACPFiles(t *testing.T, directory, referenceRoot string) error {
	...
	loaderText := strings.ReplaceAll(
		string(loader),
		"process.env.SHUTU_REFERENCE_ROOT",
...
import { LlmAdapter } from '@shutu-ai/dsh-llm'
import AgentLoop from '@shutu-ai/dsh-agent-loop'
import { mountAgentLoopTestDependencies } from '@shutu-ai/dsh-agent-loop-testkit'
import * as AcpPlugin from '@shutu-ai/dsh-acp'
...
func TestPinnedReferenceACPRuntimeExternalClient(t *testing.T) {
	referenceRoot := strings.TrimSpace(os.Getenv("SHUTU_REFERENCE_ROOT"))
	if referenceRoot == "" {
