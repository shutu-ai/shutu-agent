...
// DEEPSEEK_API_KEY returns the readable credential error (fail-closed gating
// without touching the network).
func TestRegisterWebEnabledRegistersAndValidates(t *testing.T) {
	t.Setenv("DEEPSEEK_API_KEY", "") // force the dynamic provider to fail closed
	a := makeWebApp(true)
	...
	// A valid web_search with no key: the dynamic provider remains registered
	// for live settings/credential changes, but reports its missing credential
	// without dispatching a network request.
	...
	} else if !strings.Contains(res.Output, "provider credential missing") {
		t.Errorf("output = %q, want the credential-missing hint", res.Output)
	}
	// A failed call logs nothing at the tool layer (D3: the canonical search request
	// only fires inside a provider OnRequest; credential gating happens first).
