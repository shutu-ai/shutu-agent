...
| 5 | Workflow/Ralph 协议对齐 | ✅ 已实现并复核 | Ralph 使用 dsh-compatible `summary/evidence/nextSteps/blocker` 状态语义、16K handoff 上限和严格结构化输出；工具参数使用 `maxRounds`，结构化结果包含 `runId/agentsStarted/result`；workflow 已新增外部 Node runner、`meta/script/args`、`agent/parallel/pipeline/phase/log`、RPC、取消、并发/总量/item 上限和 `workflow/*` 生命周期事件；本地 spawn provider 已提供 scoped `structured_output` 工具，`agent({schema})` 会校验并返回结构化对象；JS 默认启用，Go-native DAG 保留兼容路径，Go 核心不依赖 Node.js |
