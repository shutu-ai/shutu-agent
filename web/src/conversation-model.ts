...
import { shutuEventDetails, shutuEventString, normalizeShutuEvent } from './event-model'

/** SHUTU ConversationSnapshot-compatible surface used by the shutu web client. */
export type ShutuConversationNode =
  | ShutuUserNode
  | ShutuAssistantNode
  | ShutuToolRunningNode
  | ShutuToolResultNode
  | ShutuContextNode
  | ShutuCompactionNode
  | ShutuUnknownNode

export interface ShutuUserNode {
  readonly id: string
  readonly kind: 'user'
  readonly seq: number
...
export interface ShutuAssistantNode {
  readonly id: string
...
export interface ShutuToolResultNode {
  readonly id: string
...
export interface ShutuToolRunningNode {
  readonly id: string
...
export interface ShutuContextNode {
  readonly id: string
...
export interface ShutuCompactionNode {
  readonly id: string
...
export interface ShutuUnknownNode {
  readonly id: string
...
export interface ShutuConversationTurn {
  readonly turn: number
  readonly startTime: number | null
  readonly endTime?: number
}

export interface ShutuConversationSnapshot {
  readonly sessionId: string
  readonly chat: {
    readonly order: readonly string[]
    readonly nodes: ReadonlyMap<string, ShutuConversationNode>
    readonly timeline: {
      readonly turnOrder: readonly number[]
      readonly turns: ReadonlyMap<number, ShutuConversationTurn>
    }
  }
  readonly nodes: readonly ShutuConversationNode[]
  readonly turnTimings: ReadonlyMap<number, ShutuConversationTurn>
...
function nodeKey(node: ShutuConversationNode): string {
  return node.id
}

/**
 * Fold the shutu event view into the same outward shape used by SHUTU's
 ...
 * event vocabulary.
 */
export function projectShutuConversation(events: readonly EventView[], sessionId = ''): ShutuConversationSnapshot {
  const nodes: ShutuConversationNode[] = []
  const nodeMap = new Map<string, ShutuConversationNode>()
  const calls = new Map<string, CallState>()
  const turns = new Map<number, ShutuConversationTurn>()
  ...
  const add = (node: ShutuConversationNode): void => {
    nodes.push(node)
  ...
  for (const event of events) {
    ...
    if (event.type === 'user/message') {
      if (event.context_message) {
        add({ id: facts.id, kind: 'context', seq: event.seq, time, text: facts.text, source: event.context_source || shutuEventString(event, 'source') || 'context' })
      } else {
