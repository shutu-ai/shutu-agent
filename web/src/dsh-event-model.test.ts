...
import { dshEventId, dshEventText, normalizeDshEvent } from './dsh-event-model'
...
describe('shared DSH event model', () => {
  it('uses one identity, text and request/call normalization contract', () => {
    ...
    const facts = normalizeDshEvent(raw, null, null)

    expect(facts.id).toBe(dshEventId(raw))
    expect(facts.text).toBe(dshEventText(raw))
  ...
  it('keeps conversation and trajectory anchored to the same event IDs and tool call', () => {
    ...
    const conversation = projectDshConversation(events)
    const trajectory = projectDshTrajectoryRecords(events)
    const conversationIds = new Set(conversation.nodes.map(node => node.id))

    for (const record of trajectory) {
      if (conversationIds.has(record.id)) expect(record.id).toBe(dshEventId(record.event))
    }
    expect(conversation.nodes.find(node => node.kind === 'tool-result')).toMatchObject({
      id: dshEventId(events[4]!),
      callId: 'c-2',
      assistantId: dshEventId(events[2]!),
    })
    expect(trajectory.find(record => record.event.type === 'tool/result')).toMatchObject({
      id: dshEventId(events[4]!),
      callId: 'c-2',
      parentId: dshEventId(events[3]!),
    })
  })
  ...
  it('uses the same classification facts for context, failed tools and extensions', () => {
    ...
    const facts = events.map((item, index) => normalizeDshEvent(item, index === 2 ? 'request:r-3' : null, 1))
