...
import { deriveTrajectoryTimeline, trajectoryTimelineFocusIndexes, type ShutuTimelineMode, type ShutuTimelineModel, type ShutuTurn } from '@shutu-ai/trajectory'
import { shutuEventDetails, normalizeShutuEvent } from './event-model'

export type { ShutuTimelineMode }

export interface ShutuTrajectoryProjection {
  records: readonly ShutuTrajectoryRecord[]
  turns: readonly ShutuTurn[]
  timeline: ShutuTimelineModel | null
}

export interface ShutuTimelineMetrics {
...
export type ShutuTrajectoryRecordKind =
  | 'turn'
  | 'step'
  | 'request'
  | 'user'
...
export type ShutuTrajectoryRecordStatus = 'pending' | 'running' | 'completed' | 'failed'

export interface ShutuTrajectoryTokenUsage {
  inputTokens?: number
  outputTokens?: number
  totalTokens?: number
...
export interface ShutuTrajectoryRecord {
  readonly id: string
  readonly seq: number
  readonly version: number
  readonly eventType: string
  readonly kind: ShutuTrajectoryRecordKind
  readonly status: ShutuTrajectoryRecordStatus
  ...
  readonly usage?: ShutuTrajectoryTokenUsage
  readonly event: EventView
}

export interface ShutuTrajectoryEvent extends EventView {
...
export function summarizeShutuTimeline(events: readonly EventView[]): ShutuTimelineMetrics {
  let durationMs = 0
...
function isStructuralEvent(event: EventView): boolean {
  return normalizeShutuEvent(event, null, null).structural
}
...
function mergeAssistantStream(events: readonly EventView[]): ShutuTrajectoryEvent | null {
...
function compactTurn(turn: readonly EventView[]): readonly ShutuTrajectoryEvent[] {
  ...
  const output: ShutuTrajectoryEvent[] = []
  let inserted = false
...
/** Return the compact SHUTU trajectory view for a session. */
export function collapseShutuTrajectoryTurns(events: readonly EventView[]): readonly EventView[] {
  const compacted: ShutuTrajectoryEvent[] = []
  let turn: EventView[] = []
  const flush = (): void => {
...
function detailsOf(event: EventView): Record<string, unknown> {
  return shutuEventDetails(event)
}

function tokenUsage(event: EventView): ShutuTrajectoryTokenUsage | undefined {
  const raw = detailsOf(event).usage
  if (raw === null || typeof raw !== 'object') return undefined
  const source = raw as Record<string, unknown>
  const usage: ShutuTrajectoryTokenUsage = {
...
function recordLabel(event: EventView, kind: ShutuTrajectoryRecordKind): string {
...
/** Normalize raw events once so every SHUTU surface uses the same relationships. */
export function projectShutuTrajectoryRecords(events: readonly EventView[]): readonly ShutuTrajectoryRecord[] {
  const records: Array<Omit<ShutuTrajectoryRecord, 'childIds'>> = []
  const recordById = new Map<string, Omit<ShutuTrajectoryRecord, 'childIds'>>()
  ...
  for (const event of events) {
    const facts = normalizeShutuEvent(event, currentRequestId, currentTurn)
    const kind = facts.kind as ShutuTrajectoryRecordKind
    const id = facts.id
    ...
    const normalized: Omit<ShutuTrajectoryRecord, 'childIds'> = {
      id, seq: event.seq, version: event.version, eventType: event.type, kind,
      status: facts.status as ShutuTrajectoryRecordStatus, time: event.time, startedAt: Number.isFinite(startedAt) ? startedAt : null,
...
export function collapseShutuAssistantToolCalls(events: readonly EventView[], collapsedAssistants: ReadonlySet<number>): readonly ShutuTrajectoryEvent[] {
  const output: ShutuTrajectoryEvent[] = []
...
function turnsFromRecords(records: readonly ShutuTrajectoryRecord[]): readonly ShutuTurn[] {
  const turns: ShutuTurn[] = []
  let turn = 0
  let index = 0
  let cells: ShutuTurn['groups'][number]['cells'] = []
  const flush = (): void => {
...
export function toShutuTurns(events: readonly EventView[]): readonly ShutuTurn[] {
  return turnsFromRecords(projectShutuTrajectoryRecords(events))
}

export function projectShutuTrajectory(events: readonly EventView[], mode: ShutuTimelineMode = 'sequence'): ShutuTrajectoryProjection {
...
export function focusShutuTrajectory(events: readonly EventView[], range: { start: number; end: number }, mode: ShutuTimelineMode = 'sequence'): ReadonlySet<number> {
  return trajectoryTimelineFocusIndexes(toShutuTurns(events), range, mode)
}
