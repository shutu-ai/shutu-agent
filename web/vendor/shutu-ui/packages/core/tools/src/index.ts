 ...
 * execution pipeline.
 * @module @shutu-ai/tools
 */

import { Context, Service } from '@shutu-ai/cordis'
import z from '@shutu-ai/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeOf, scopeTarget } from '@shutu-ai/scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@shutu-ai/scope'
import type { CallId, ContentBlock, ToolSchema } from '@shutu-ai/llm'
import { assertNever, deepFreeze, HarnessError } from '@shutu-ai/llm'
import type { Agent } from '@shutu-ai/agent'
import { snapshotJsonValue } from '@shutu-ai/session'
import type { JsonValue, UserMessage } from '@shutu-ai/session'
import type { ToolProviderResult } from '@shutu-ai/system-prompt'
import type { CodeRuntime } from '@shutu-ai/code-runtime'
 ...
 * instead of deriving them: the seam's `shutu-code-runtime` README pair, its
...
export type { JsonValue } from '@shutu-ai/session'
...
// lives in its own UI-facing module; re-export it so `@shutu-ai/tools`
...
declare module '@shutu-ai/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@shutu-ai/scope`): agent-scoped listeners receive only that agent's calls.
     ...
     * Scope-filtered dispatch (`@shutu-ai/scope`): agent-scoped listeners receive only that agent's dispatches.
     ...
     * Scope-filtered dispatch (`@shutu-ai/scope`): keyed by `exec.agent`.
...
export interface ToolDefinition extends ToolSchema {
   ...
   * Enforced by `@shutu-ai/tool-call-timeout-policy` (a `tools/execute` wrapper); it
 ...
 * @internal
 */
export const TOOL_RUNTIME_SCHEDULER: unique symbol = Symbol('@shutu-ai/tools.scheduler')
...
export class ToolRuntime extends Service {
  ...
  /** Internal staged view consumed by `shutu-agent-loop`'s parallel scheduler. */
  ...
  private sdkSection(): { name: string; order: number; text: (context: { scope?: ScopeKey }) => string } {
    return {
      ...
      text: (context) => {
        ...
        if (render === undefined) throw new Error(`shutu-tools: no SDK renderer for ${runtime.language}`)
  ...
  private requireCodeRuntime(mode: ToolPresentationMode): CodeRuntime {
    ...
    if (!runtime) {
      throw new Error(`shutu-tools: mode "${mode}" requires a code runtime — load a ctx.codeRuntime implementation (e.g. @shutu-ai/code-runtime-worker-thread) or set tools mode to "native"`)
    }
    if (!Object.hasOwn(SDK_RENDERERS, runtime.language)) {
      const known = Object.keys(SDK_RENDERERS).map(name => JSON.stringify(name)).join(', ')
      throw new Error(`shutu-tools: no SDK renderer registered for runtime language ${JSON.stringify(runtime.language)} (known: ${known})`)
    }
    return runtime
  }
   ...
   * `shutu-agent-tool-presentation` exists for, and reading the deployment default would
...
function createExecutionToken(): ToolExecutionToken {
  return Symbol('shutu.tool.execution') as ToolExecutionToken
}
