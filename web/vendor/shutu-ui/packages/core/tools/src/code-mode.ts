 ...
 * @module @shutu-ai/tools/src/code-mode
 */

import { CallId, createUserMessage, HarnessError } from '@shutu-ai/llm'
import type { ContentBlock } from '@shutu-ai/llm'
import type { CodeBindingFunction, CodeRunResult, CodeRuntime } from '@shutu-ai/code-runtime'
import { snapshotJsonValue } from '@shutu-ai/session'
import type { JsonValue } from '@shutu-ai/session'
...
function resolveFlavor(peekRuntime: () => CodeRuntime | undefined): RunCodeFlavor {
  ...
  if (!Object.hasOwn(RUN_CODE_FLAVORS, runtime.language) || flavor === undefined) {
    ...
    throw new Error(`shutu-tools: no run_code schema flavor registered for runtime language ${JSON.stringify(runtime.language)} (known: ${known})`)
  }
  return flavor
}
