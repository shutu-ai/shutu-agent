 ...
 *
 * @module @shutu-ai/agent
 */

import { Context, FiberState, getTraceable, Service, symbols } from '@shutu-ai/cordis'
import type { Fiber } from '@shutu-ai/cordis'
import { AsyncLocalStorage } from 'node:async_hooks'
import { isPromise } from 'node:util/types'
import { scopeTarget } from '@shutu-ai/scope'
import type { Scoped } from '@shutu-ai/scope'
import type { SessionEvent, SessionId } from '@shutu-ai/session'
import type { TypertContext, TypertLookup } from '@shutu-ai/typert-protocol'
...
declare module '@shutu-ai/typert-protocol' {
...
declare module '@shutu-ai/cordis' {
  interface Context {
     ...
     * `shutu-scope` tag while retaining it, so this field is DX context rather
...
export interface CreateAgentOptions {
   ...
   * {@link CreateSessionOptions.meta} in shutu-session (the internal-only
 ...
 * via {@link AgentRegistry.setFactory}. Kept on the `shutu-agent` interface so
 * consumers (e.g. the ACP bridge) program against `ctx.agents` without
 * depending on the concrete `shutu-agent-loop` package.
 */
 ...
 * (`@shutu-ai/agent-loop`), registered via {@link setFactory}.
 *
  ...
  constructor(ctx: Context) {
    ...
    ctx.inject(['typert'], (typeCtx) => {
      typeCtx.typert.lookups.register('agent', {
        parameter: 'agent',
        wire: 'agentId',
        hostTypeSymbol: '@shutu-ai/agent#Agent',
        wireTypeSymbol: '@shutu-ai/session/types#SessionId',
