 ...
 *
 * @module @shutu-ai/scope
 */

import type { Context, Fiber } from '@shutu-ai/cordis'
import { Context as CordisContext } from '@shutu-ai/cordis'
...
const kScope = Symbol('shutu.scope')
...
function linkScopeParent(key: ScopeKey, parent: ScopeKey): void {
  for (let cursor: ScopeKey | undefined = parent; cursor !== undefined; cursor = scopeParents.get(cursor)) {
    if (cursor === key) throw new Error('shutu-scope: scope parent link would form a cycle')
  }
...
export function bindScopeParent(key: ScopeKey, parent: ScopeKey): ScopeParentBinding {
  if (scopeParents.has(key)) {
    throw new Error('shutu-scope: scope key is already bound to a parent; re-linking requires the binding returned by the original bind')
  }
