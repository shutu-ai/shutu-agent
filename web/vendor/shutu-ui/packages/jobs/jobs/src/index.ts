 ...
 * process-local registry lives in `@shutu-ai/jobs-local`.
 * @module @shutu-ai/jobs
 */
...
export abstract class JobRegistry extends Service {
  constructor(ctx: Context) {
    ...
    if (new.target === JobRegistry) {
      throw new Error('@shutu-ai/jobs is the abstract job registry seam; load an implementation such as @shutu-ai/jobs-local instead')
    }
    super(ctx, 'jobs')
  }
