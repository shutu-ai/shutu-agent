...
import { SessionId, type SessionId as SessionIdType } from '@shutu-ai/session'
...
export function parseSessionReferenceText(text: string): ParsedSessionReferenceText {
  ...
  const pattern = /@\[((?:\\.|[^\\\]])*)\]\((shutu-session:[^\s)]*)\)|(shutu-session:[A-Za-z0-9_-]+)/gu
