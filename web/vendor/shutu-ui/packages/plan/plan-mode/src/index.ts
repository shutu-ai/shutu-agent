 ...
 *
 * @module @shutu-ai/plan-mode
 */

import { Context, Service } from '@shutu-ai/cordis'
import { z as zod } from 'zod'
import type { ZodType } from 'zod'
import type { Agent, PreStepDecision } from '@shutu-ai/agent'
import { createUserMessage } from '@shutu-ai/llm'
import type { Session, SessionEvent, UserMessage } from '@shutu-ai/session'
...
declare module '@shutu-ai/session/types' {
  ...
  constructor(ctx: Context, config: PlanModeConfig = { section: '' }) {
    ...
    ctx.on('agent/pre-step', async (
      ...
      try {
        ...
        ctx.logger.warn('shutu-plan-mode: failed to append selected plan mode at step start: %o', error)
        return decision
      }
    ...
    ctx.effect(() => () => { disposed = true }, 'shutu-plan-mode: close service lifetime')
