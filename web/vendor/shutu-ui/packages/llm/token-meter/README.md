...
```yaml
- name: '@shutu-ai/token-meter'
- name: '@shutu-ai/compaction-basic'
```

Both plugins have usable defaults. The meter remains independent of model routing and optional compaction. A deployment configures capacity on its LLM adapter and compaction policy on `shutu-compaction-basic`.

## Model Experience

Indirectly, through consumers such as `shutu-compaction-basic`; the service itself adds no prompt, message, schema, tool, or model call.
