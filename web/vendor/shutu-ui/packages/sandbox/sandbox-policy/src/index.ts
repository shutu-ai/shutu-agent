 ...
 * remain session-free.
 *
 * @module @shutu-ai/sandbox-policy
 */

import { resolve as resolvePath } from 'node:path'
import { Context, Service } from '@shutu-ai/cordis'
import z from '@shutu-ai/schemastery'
import type {} from '@shutu-ai/agent'
import { canonicalPath, type SandboxExecutionPolicy, type SandboxMode } from '@shutu-ai/sandbox'
import type { Session } from '@shutu-ai/session'
import type {} from '@shutu-ai/system-prompt'
...
function renderPolicyContext(policy: SandboxExecutionPolicy): string {
  switch (policy.mode) {
    case 'read-only':
      return 'Current SHUTU file policy: read-only. Any available operation enforced by the SHUTU file sandbox cannot modify files in the standing mode. Do not refuse a required modification from this policy alone: try an available tool normally and follow any denial and escalation guidance it returns.'
    case 'workspace-write':
      return `Current SHUTU file policy: workspace-write. Any available operation enforced by the SHUTU file sandbox may modify files under the session workspace: ${JSON.stringify(policy.workspaceRoot)}. Some platform temporary areas may also be writable.`
    case 'danger-full-access':
      return 'Current SHUTU file policy: danger-full-access. The SHUTU file sandbox does not restrict file modifications by available operations.'
