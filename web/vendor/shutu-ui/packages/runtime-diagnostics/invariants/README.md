...
Session itself owns immutable, surface-valid log storage in every composition: it takes one lossless JSON snapshot of each candidate, validates complete cited source-event coverage and positional replacement, restricts `tool/result` replacement to one current result's `content`, deep-freezes the accepted record, and exposes the log through immutable array snapshots. The `shutu-session` invariant companion checks the remaining cross-record rules that Session does not own.
...
| Companion | Checks |
|---|---|
| `shutu-session`, `shutu-agent`, `shutu-scope`, `shutu-agent-loop` | Session enclosure and call/result trace, agent-status transitions, inbox FIFO conservation, scoped subjects, and model-request reconstruction. |
| `shutu-llm`, `shutu-llm-retry`, `shutu-tools`, `shutu-system-prompt` | Stream grammar, durable retry position and bounds, tool-pipeline stages and frozen results, and authoritative prompt-assembly data. |
| `shutu-compaction`, `shutu-hook-protocol`, `shutu-sandbox-policy` | Durable compaction and hook pairing, compaction metadata, and sandbox-mode vocabulary. |
| `shutu-fs`, `shutu-subagent`, `shutu-workflow` | Filesystem event identity, provider/child pairing, and workflow/agent lifecycle identity. |
| `shutu-goal`, `shutu-goal-round-driver` | Durable goal source/content agreement, revision and lifecycle transitions, timestamps, sequential admitted rounds, and reconstructed continuation prompts. |
| `shutu-permission-presets`, `shutu-user-approval` | Active-preset references and approval asked/decided audit pairing. |
| `shutu-jobs`, `shutu-tool-todo` | Task snapshot lifecycle/ownership fields and durable whole-list todo structure. |
| `shutu-time-context` | Durable clock readings agree with the session's open turn and next pre-step position and elapsed baseline; rendered time parses and does not postdate its event. |
...
import InvariantRegistry from '@shutu-ai/invariants'
import * as SessionInvariant from '@shutu-ai/session/invariant'

declare const ctx: Context

ctx.plugin(InvariantRegistry, {
  enabled: true,
  package_allowlist: ['^@shutu-ai/'],
  package_blocklist: ['^@shutu-ai/agent-loop$'],
})
