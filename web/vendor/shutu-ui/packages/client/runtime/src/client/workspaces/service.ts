...
export class WorkspaceRuntime implements IWorkspaces {
  ...
  startSession(workspaceId?: WorkspaceId): void {
    ...
    // workspace. An explicit named-workspace choice still wins.
    ...
    if (target === undefined) {
      // With no current session and no workspace baseline, land in ungrouped
      // using the host's configured default directory.
      const attempt = this.sessions.create({})
      void attempt.then(
        sessionId => { this.sessions.open(sessionId) },
        (reason: unknown) => { console.warn('new session failed:', reason) },
