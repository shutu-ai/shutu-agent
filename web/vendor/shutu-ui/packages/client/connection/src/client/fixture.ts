...
import {
  ...
  createUserMessage,
  isTokenDelta,
} from '@shutu-ai/llm/message'
import { CallId } from '@shutu-ai/llm/brand'
import type {
  AssistantMessage,
  ContentBlock,
  MessageSource,
...
import type { AttachmentIdType, ImageAttachmentRef } from '@shutu-ai/attachment'
import type {
  SessionEvent,
  SessionId,
  TodoItem,
} from '@shutu-ai/session/types'
// Type-only: the brand constructor is host-side; the fixture casts at its
// wire-fabrication boundary (the schema layer's one-cast-point posture).
import type { CommandId } from '@shutu-ai/commands/brand'
import type { CommandDescriptor, CommandExecution, CommandResult } from '@shutu-ai/commands/types'
import { deriveEventMessage, foldSurface } from '@shutu-ai/session/surface'
import type {
  ApiProxy, ClientRequest, ClientResponse, HistoryEntry, HostFrame, MuxFrame, RpcReceipt,
  ModelProviderGroup, ModelSelection, RpcRequest, RpcResponse, RpcResult, ServerRequest, ServerResponse, SessionSummary,
  ToolCallView, ToolEventView, ToolResultView, WorkspaceId, WorkspaceView,
} from './api.ts'
import type { RequestPayload, ResponseValue, RpcMethodMap } from '@shutu-ai/host-apiproxy/api'
 ...
 * presenter's shape (see formatGrepOutput in shutu-tool-fs-search): a
 ...
 * formatGlobOutput in shutu-tool-fs-search).
 */
...
const WEB_SEARCH_RESULT: Omit<Extract<ToolResultView, { card: 'web'; kind: 'search' }>, 'card' | 'kind'> = {
  ...
  sources: [
    {
      url: 'https://github.com/shutu-ai/deepseek-harness',
...
/** Fixture-local mirror of the goal projection value (shutu-goal's GoalProjection shape). */
...
function createFixtureWorld(options: FixtureOptions): FixtureWorld {
  ...
  const fixturePresets = new Map<string, { trust: 'system' | 'user'; content: string }>([
    ['standard', { trust: 'system', content: "- id: tool-bash\n  name: '@shutu-ai/tool-bash'\n" }],
    ['minimal', { trust: 'system', content: "- id: tool-web-search\n  name: '@shutu-ai/tool-web-search'\n" }],
    ['my-agent', { trust: 'user', content: "- id: tool-read\n  name: '@shutu-ai/tool-read'\n" }],
  ])
      ...
      const value = sessions
        ...
        .map((item) => {
          ...
          return {
            ...
            mention: `@[${label}](shutu-session:${encoded})`,
          }
        })
