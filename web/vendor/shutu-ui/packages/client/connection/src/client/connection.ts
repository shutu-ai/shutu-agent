...
export interface ConnectionConfig {
  ...
  backoffMaxMs?: number
  /** Cap on waiting for both streams' onOpen before onConnected, in ms. The strict hanshutuake
...
/** Coarse connection state for the UI: 'connected' after each generation's hanshutuake,
...
export class ConnectionController {
  ...
  private async loop(): Promise<void> {
    while (this.running) {
      ...
      try {
        // Strict readiness hanshutuake: describe proves unary reachability, onOpen
        ...
        if (ac.signal.aborted) throw new Error('generation aborted during readiness hanshutuake')
        this.attempt = 0
