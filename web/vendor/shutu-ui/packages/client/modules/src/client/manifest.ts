 ...
 * `__SHUTU_BOOT__` wire types, the boot-manifest parser, and the boundaries around
 * {@link ClientModuleSystem}. The package root is the host-side service that
 * composes the wire.
 */

import type {} from '@shutu-ai/cordis'
 ...
 * metadata (the authoritative edges live in each package's `shutu.client`
...
/** The composed client entry graph the host injects as `window.__SHUTU_BOOT__`. */
...
/**
 * Validate an optional string-array field read from a `shutu.client` declaration
...
export function parseBootManifest(wire: unknown): BootManifest {
  if (typeof wire !== 'object' || wire === null) {
    throw new Error('client-modules: window.__SHUTU_BOOT__ is missing or not an object')
  }
