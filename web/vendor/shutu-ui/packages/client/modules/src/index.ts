/**
 * Node half of the client module system (`shutu.client` dual-face package): scans
 * the host Loader's entries for packages declaring `shutu.client`, composes the
 * `window.__SHUTU_BOOT__` entry graph (wire single source: {@link WebBootEntry}
 ...
 * @module @shutu-ai/client-modules
 */
...
import { Service } from '@shutu-ai/cordis'
import type { Context } from '@shutu-ai/cordis'
import type {} from '@shutu-ai/cordis-plugin-loader'
import type {} from '@shutu-ai/host-webserver'
...
/** package.json `shutu.client` declaration fields, validated one by one after reading the file. */
interface ShutuClientDeclaration {
  inject?: string[]
  platform: string
...
/** Resolved package metadata for one `shutu.client` package (cached per name, never expires). */
...
/** Narrow an unknown parsed JSON value to the `shutu.client` declaration, throwing on malformed fields. */
function parseShutuClient(pkgName: string, value: unknown): ShutuClientDeclaration | undefined {
  if (value === undefined) return undefined
  if (typeof value !== 'object' || value === null) {
    throw new Error(`client-modules: ${pkgName} has a non-object shutu.client declaration`)
  }
  const decl = value as Record<string, unknown>
  if (typeof decl.platform !== 'string') {
    throw new Error(`client-modules: ${pkgName} shutu.client.platform must be a string`)
  }
  const inject = optionalStringArray(pkgName, 'shutu.client.inject', decl.inject)
  const external = optionalStringArray(pkgName, 'shutu.client.external', decl.external)
  if (decl.immediately !== undefined && typeof decl.immediately !== 'boolean') {
    throw new Error(`client-modules: ${pkgName} shutu.client.immediately must be a boolean`)
  }
  return {
    ...
    for (const name of entry.external ?? []) {
      ...
      if (dependency === entry) {
        throw new Error(
          ...
          + '— a row must not declare its own package in shutu.client.external',
        )
      }
...
const CLIENT_MODULES_ID = '@shutu-ai/client-modules'

/** Dynamic package whose ordinary client bundle must be registered before plugin boot starts. */
const CLIENT_RUNTIME_ID = '@shutu-ai/client-runtime'
...
window.__ModuleLoader__={
  ...
  const preload = PARSER_PRELOAD_IDS.map(id => graph.entries.find(entry => entry.id === id))
    ...
    .join('')
  const script = `${queue}${preload}<script>window.__SHUTU_BOOT__ = ${json}</script>`
...
/**
 * The web plugin table service: incremental `shutu.client` scan + wire composition
...
export class ClientModuleRegistry extends Service {
  ...
  // subpath rows — or a package without a web `shutu.client` declaration) are
   ...
   * @returns the graph served as `window.__SHUTU_BOOT__`.
   */
  graph(): WebBootGraph {
  ...
  private resolveMeta(pkgName: string): PkgMeta | null {
    ...
    const shutu = pkg.shutu
    const decl = parseShutuClient(
      pkgName,
      shutu !== null && typeof shutu === 'object' ? (shutu as Record<string, unknown>).client : undefined,
    )
    ...
    if (clientRel === undefined) {
      throw new Error(`client-modules: ${pkgName} declares shutu.client but exports no "./client" bundle`)
    }
    const meta: PkgMeta = {
