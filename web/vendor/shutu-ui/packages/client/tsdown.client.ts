 ...
 */
const CSS_VIRTUAL_PREFIX = '\0shutu-css:'
const GLOBAL_CSS_VIRTUAL_PREFIX = '\0shutu-global-css:'
const INLINE_CSS_VIRTUAL_PREFIX = '\0shutu-inline-css:'
 ...
 * Everything else under @shutu-ai/* is either a module-table entry
 * (external) or a leak the purity gate rejects.
 */
export const INLINE_SAFE = /^@shutu-ai\/shutu-(host-apiproxy|file-reference|session|llm|tools|brand)(\/|$)/

/**
 * Vendored framework libraries: rescoped into @shutu-ai, so the gate below
 ...
 */
const VENDORED_LIBRARY = /^@shutu-ai\/(cosmokit|schemastery)(\/|$)/

/** Generated descriptor/codec contribution with no shared runtime identity. */
const GENERATED_REMOTE = /^@shutu-ai\/shutu-[a-z0-9]+(?:-[a-z0-9]+)*\/remote$/
...
export function clientBundle(
  ...
  return ({ env }) => {
    const face = buildFace(env?.SHUTU_BUILD_FACE)
...
export function clientOnly(configs: readonly UserConfig[]): BuildFaceConfig {
  return ({ env }) => buildFace(env?.SHUTU_BUILD_FACE) === 'host'
...
function buildFace(value: unknown): BuildFace {
  ...
  throw new Error(`tsdown: --env.SHUTU_BUILD_FACE must be host or client, received ${String(value)}`)
}
...
interface WorkspaceManifest {
  ...
  readonly shutu?: { readonly client?: { readonly external?: unknown } }
}
...
/**
 * Module-table specifiers one `shutu.client` declaration requests. Matching is
 ...
 * @param declaration - the package's `shutu.client` object.
...
export function requestedExternals(
  ...
  return new Set(optionalStringArray(subject, 'shutu.client.external', declaration.external) ?? [])
}

/**
 * Module-table specifiers one package requests. The shell baseline is implicit
 * for every dynamic bundle; `shutu.client.external` only adds package-specific
...
function clientExternals(id: string): ReadonlySet<string> {
  ...
  const externals = new Set([
    ...
    ...requestedExternals(id, workspaceManifest(id).shutu?.client ?? {}),
  ])
...
function clientConfig(id: string, entry: string): UserConfig {
  ...
  return {
    ...
    plugins: [{
      ...
      // inline, and every other @shutu-ai value import is a build error — a
      ...
      name: 'shutu-client-bundle-purity',
      resolveId(source: string) {
        if (!source.startsWith('@shutu-ai/')) return null
        if (isRequested(source)) return null // requested module-table row: external wins
        if (VENDORED_LIBRARY.test(source)) return null // vendored library: inline, no shared identity
        if (INLINE_SAFE.test(source) || GENERATED_REMOTE.test(source)) return null // wire contribution: inline is the point
        throw new Error(
          `client bundle purity: "${source}" is not in the default client externals or ${id}'s shutu.client.external, an inline-safe wire layer, or a generated /remote contribution — `
...
const STATIC_LINKED_PLUGIN = 'shutu-static-linked-external'
