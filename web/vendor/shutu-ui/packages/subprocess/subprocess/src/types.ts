 ...
 * @module shutu-subprocess/types
 */

import type { Readable, Writable } from 'node:stream'

/** Namespace prefix reserved for DeepSeek Harness-managed child environment facts. */
export const SHUTU_ENV_PREFIX = 'SHUTU_' as const

/** One environment key inside the managed {@link SHUTU_ENV_PREFIX} namespace. */
export type ShutuEnvironmentKey = `${typeof SHUTU_ENV_PREFIX}${string}`

/** Trusted DeepSeek Harness variables for one child-process execution. */
export type ShutuEnvironment = Readonly<Record<ShutuEnvironmentKey, string>>
 ...
 * not a hidden subprocess-service default — decides them (the `shutu-shell`
...
export interface SubprocessSpawnSpec {
   ...
   * entry or current `SHUTU_*` fact survives the scrub; `undefined` is a
 ...
 * to `TerminalSignal` in `@shutu-ai/terminal` without a cross-seam dependency;
