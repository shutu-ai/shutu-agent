...
```ts
import { clampTimeout, deadline, idleWatchdog, MAX_TIMER_DELAY_MS, timeoutOf, TimeoutReason } from '@shutu-ai/timeout'
```
...
```ts
import { deadline, timeoutOf } from '@shutu-ai/timeout'
...
Indirectly, through consumers such as `shutu-tool-call-timeout-policy`, which may replace a provider result with a retained timeout error or suppress a late result.
