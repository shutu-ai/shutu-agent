...
`resolveShutuHome()` resolves the single-root DeepSeek Harness home. Precedence, highest first: an explicit configured path, `$SHUTU_HOME`, then `~/.shutu`. The harness keeps all user data under one root.

`shutuHomePath(...segments)` joins child segments onto that resolved home with Node's platform path rules. With no segments it returns the home itself.

`shutuHomeDisplay()` names an active root symbolically for user-facing paths: `~/.shutu` for the default home, `$SHUTU_HOME` for any configured home. It never leaks an absolute machine path.

`SHUTU_HOME_DIR_NAME` owns the default user-data directory name: `.shutu`.

`defaultShutuHome()` returns the default DeepSeek Harness home by joining the operating-system home directory with `.shutu`, using Node's platform path rules.
