...
import type { AttachmentStore, ImageAttachmentRef } from '@shutu-ai/attachment'
import type { SessionLineageNode, SessionQueryEngine } from '@shutu-ai/session-query'
import type { SessionId, SessionStore } from '@shutu-ai/session'
import type { SessionPersistence, SessionRawArtifact } from '@shutu-ai/session-persistence'
...
export function sessionLogZipFilename(sessionId: string): string {
  return `shutu-session-${safeSessionIdSegment(sessionId)}.zip`
}
