...
import { installModelSelection } from '@shutu-ai/agent'
import type { Agent, ModelSelection, ModelSelectionRef, AgentOptions, AgentStatus } from '@shutu-ai/agent'
import type {} from '@shutu-ai/agent-presets/types'
import { AttachmentError, admitEncodedImages } from '@shutu-ai/attachment'
import type { ImageAttachmentRef } from '@shutu-ai/attachment'
import { contentHasImage, createUserMessage, freezeMessage, ReasoningEffortId } from '@shutu-ai/llm'
import { errorChain } from '@shutu-ai/llm'
import type { ContentBlock, MessageSource } from '@shutu-ai/llm'
import { isAppendSurfaceEvent, isJsonValue } from '@shutu-ai/session'
import type { JsonValue, Session, SessionEvent, SessionEventMap, SessionHeader, SessionId, UserMessage } from '@shutu-ai/session'
import type { SessionPersistence } from '@shutu-ai/session-persistence'
import { SessionQueryError, type SessionSearchCursor } from '@shutu-ai/session-query'
import { SubagentError } from '@shutu-ai/subagent'
import type { SubagentListEntry as CatalogSubagentListEntry } from '@shutu-ai/subagent'
import { isUserInvocable } from '@shutu-ai/skill'
import type { Workspace, WorkspaceRecord } from '@shutu-ai/workspace'
import {
  workspaceDomainState, workspaceRecord, WorkspaceId as brandWorkspaceId,
  WorkspaceMoveInvalidError, WorkspaceOrderInvalidError, WorkspaceUnknownSessionError,
} from '@shutu-ai/workspace'
// Type-only: brings the `ctx.tools` Context merge into this program (viewFor reads presenters).
import {
  InvalidPresetIdError, PresetExistsError, PresetMountError,
  PresetNotWritableError, resolveSessionPreset, UnknownPresetError,
} from '@shutu-ai/agent-presets'
import type { PresetBearingSession } from '@shutu-ai/agent-presets'
import type {} from '@shutu-ai/tools'
import type {
...
import type { SessionRawArtifact } from '@shutu-ai/session-persistence'
import {
...
import type {} from '@shutu-ai/session-projection'
// Type-only: resolves `ctx.get('tasks')` to the background job registry.
import type {} from '@shutu-ai/jobs'
import type { JobSnapshot } from '@shutu-ai/jobs'
// Type-only: resolves `ctx.get('sessionProjectionCache')` (the cold listing column).
import type {} from '@shutu-ai/session-projection-cache'
// GoalError narrows domain rejections to their stable codes at the wire boundary.
import { GoalError } from '@shutu-ai/goal'
import type { GoalRef as CoreGoalRef } from '@shutu-ai/goal'
// Type-only edges: resolve the command-change stream and `ctx.get('skills')`.
import type {} from '@shutu-ai/commands'
...
import type {} from '@shutu-ai/cordis-host-runner/types'
import type {} from '@shutu-ai/skill'
// The settings/credentials seams: brand guards run at this wire boundary; the
// service reads stay optional (`ctx.get`) so a composition without either
// provider still serves every other domain.
import { SettingsConflictError, settingsNamespace } from '@shutu-ai/settings'
import type { SettingsDescriptor, SettingsNamespace, SettingsPathOp } from '@shutu-ai/settings'
import { credentialRef } from '@shutu-ai/credentials'
// Value edge: the rename impl narrows the title service's validation failure; the import also resolves `ctx.get('sessionTitle')`.
import { SessionTitleInvalidError } from '@shutu-ai/session-title'
import type { CallId } from '@shutu-ai/llm/brand'
import type { ScopeKey } from '@shutu-ai/scope'
import type { ApprovalOutcome, ApprovalRequestId } from '@shutu-ai/user-approval'
// Side-effect type import: resolves the `approval/request` waterfall and
// `ctx.get('approval')` without a value dependency on the seam (optional composition).
import type {} from '@shutu-ai/user-approval'
...
} from '@shutu-ai/user-questions'
import { UserQuestionError } from '@shutu-ai/user-questions'
import { DirectoryPickerError } from '@shutu-ai/host-directory-picker'
import {
...
function projectionsUnavailableError(): RpcError {
  return {
    code: 'internal',
    message: 'subagent catalog is unavailable: this deployment does not mount the sessionProjections registry (load @shutu-ai/session-projection)',
    details: {},
  }
}
...
export function createApiProxy(ctx: Context, defaults: ApiProxyDefaults): ApiProxy {
  ...
  // attachment Service Definition: shutu-llm depends on shutu-attachment, so the
  ...
  function goalServiceFor(agent: Agent): NonNullable<ReturnType<typeof ctx.get<'goals'>>> | { error: RpcError } {
    ...
    if (goals === undefined) {
      return { error: { code: 'internal', message: 'goal service is absent: neither this session\'s agent preset nor the host composition mounts @shutu-ai/goal', details: {} } }
    }
    return goals
  }
  ...
  function settingsAbsent(): RpcError {
    return { code: 'internal', message: 'settings service is absent: this deployment does not mount a settings provider (e.g. @shutu-ai/settings-file) in its composition', details: {} }
  }
  ...
  function credentialsAbsent(): RpcError {
    return { code: 'internal', message: 'credentials service is absent: this deployment does not mount a credential provider (e.g. @shutu-ai/credentials-local) in its composition', details: {} }
  }
  ...
  return {
    sessions: {
      ...
      async search(request, signal) {
        ...
        if (sessionQuery === undefined) {
          return err(request, {
            code: 'internal',
            message: 'session search is unavailable: this deployment does not mount @shutu-ai/session-query',
            details: {},
          })
        }
        try {
    ...
    agentPresets: {
      ...
      // Authoring is privileged (see PRIVILEGED_METHODS in shutu-client-connection):
    ...
    skills: {
      ...
      async list(request) {
        ...
        // composition mounts shutu-skill, not an empty catalog. `ctx.get` also
        ...
        if (skillRegistry === undefined) {
          return err(request, { code: 'internal', message: 'skill registry is absent: neither this session\'s agent preset nor the host composition mounts @shutu-ai/skill', details: {} })
        }
