/**
 * Package-owned invariant companion for `@shutu-ai/host-webserver`.
 * @module @shutu-ai/host-webserver/invariant
 */
...
const PACKAGE_NAME = '@shutu-ai/host-webserver'
...
const install: InvariantInstaller = (ctx, fail) => {
  ctx.on('internal/plugin', () => {
    ...
    const probe = { kind: 'exact' as const, path: '/__shutu_invariant_probe__', handler: () => {} }
    try {
      server.register(probe)()
      server.register(probe)()
      const upgradeProbe = { path: '/__shutu_invariant_upgrade_probe__', handler: () => {} }
