...
Public request, value, version, and failure types are exported from the package root and `@shutu-ai/message-feedback/types`; [`src/types.ts`](src/types.ts) is their source.
...
```yaml
- id: message-feedback
  name: '@shutu-ai/message-feedback'
  config:
    maxNoteBytes: 8192
```
