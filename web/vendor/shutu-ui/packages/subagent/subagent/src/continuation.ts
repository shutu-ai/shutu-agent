...
import { boundContextSummary, createUserMessage, errorChain } from '@shutu-ai/llm'
import type { ContentBlock, MessageId, MessageSource } from '@shutu-ai/llm'
import { SessionId } from '@shutu-ai/session'
import type { SessionEvent } from '@shutu-ai/session'
import type { SessionPersistence } from '@shutu-ai/session-persistence'
import type { ToolRestriction } from '@shutu-ai/tools'
...
export interface ContinuableStartSpec {
   ...
   * before child materialization without a second identity hanshutuake.
   */
...
export class SubagentContinuationManager {
  ...
  private requirePersistence(): SessionPersistence {
    ...
    if (persistence === undefined) {
      throw new SubagentError(
        'continuable subagents require session persistence (load a shutu-session-persistence backend)',
