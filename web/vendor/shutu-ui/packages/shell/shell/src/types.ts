 ...
 * to `@shutu-ai/jobs`; this seam exposes only process handles. The
 ...
 * root.
 * @module shutu-shell/types
 */

import type { SandboxEnforcement, SandboxExecutionPolicy, SandboxMode } from '@shutu-ai/sandbox'
import type { CollectedOutput, ShutuEnvironment } from '@shutu-ai/subprocess'

export { SHUTU_ENV_PREFIX } from '@shutu-ai/subprocess'
export type { CollectedOutput, ShutuEnvironment, ShutuEnvironmentKey } from '@shutu-ai/subprocess'
...
export interface ShellExecRequest {
   ...
   * scrub. Managed facts belong in {@link shutuEnv}, which merges after this
  ...
  /**
   * Harness-owned `SHUTU_*` variables for this execution (typed to managed
   * keys). Executors discard ambient `SHUTU_*` entries before merging this
   ...
   */
  shutuEnv?: ShutuEnvironment | undefined
...
export interface ShellExecSpec {
   ...
   * {@link ShellExecRequest.env}; {@link shutuEnv} still merges after them.
   * OPTIONAL on the spec for the same reason as `stdin`: absent means no
   * ordinary extra environment.
   */
  env?: Record<string, string> | undefined
  /** Managed `SHUTU_*` snapshot (typed to managed keys); merges after {@link env}. */
  shutuEnv?: ShutuEnvironment | undefined
