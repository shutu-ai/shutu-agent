 ...
 * @module @shutu-ai/agent-presets
 */

import { stat } from 'node:fs/promises'
import { Context, Service } from '@shutu-ai/cordis'
import z from '@shutu-ai/schemastery'
import { bindScopeParent, createScope, scopeOf, type Scope, type ScopeKey, type ScopeParentBinding } from '@shutu-ai/scope'
// Type-only: resolves the `agent/created` lifecycle event this service watches.
import type {} from '@shutu-ai/agent'
import { settingsNamespace, type SettingsScope, type default as SettingsService } from '@shutu-ai/settings'
import { shutuHomePath } from '@shutu-ai/home-paths'
...
export class AgentPresets extends Service {
  ...
  constructor(ctx: Context, public config: Config) {
    ...
    this.resolvedRoots = config.includeUserRoot
      ? [...config.roots, { path: shutuHomePath(USER_PRESET_DIR), trust: 'user' }]
      : [...config.roots]
   ...
   * scope key. The binding is shutu-scope's only re-link capability; holding it
   ...
   * from the agent's mount — shutu-scope's only re-link authority. An agent
  ...
  private async ensureStanding(preset: AgentPreset): Promise<StandingMount> {
    ...
    if (pending !== undefined) {
      ...
      // it is gone. The subtree is not inert — `shutu-skill-filesystem` watches its
