...
func Classifications() []Capability {
	...
	capabilities := []Capability{
		...
		required("ctx.credentials", "dedicated value boundary with write-only Web projection", "internal/credential; cmd/sta", "credential rotation/rollback/redaction tests"),
		...
		requiredWeb("ctx.storageDomain", "one lifecycle-bound domain facade over durable typed state", "cmd/sta; internal/store", "composition cold-restart tests"),
		...
		requiredWeb("ctx.workspaceRegistry", "workspace identity, grouping and ordering boundary", "internal/store; cmd/sta; internal/webserver", "workspace lifecycle/order tests"),
		...
		requiredWeb("ctx.fileReferences", "cwd-bounded path-only candidate discovery", "cmd/sta; internal/webserver", "file-reference wire tests"),
		...
		required("ctx.sessionTitle", "deterministic fallback plus bounded async model title", "cmd/sta/title.go", "title fallback/restart tests"),
		required("ctx.systemPrompt", "one prompt/tool projection assembled per runtime", "internal/prompt; cmd/sta", "prompt/catalog snapshot tests"),
		...
		required("ctx.planMode", "durable plan mode with approved exit boundary", "internal/plan; cmd/sta", "plan mode runtime tests"),
		requiredWeb("ctx.agentPresets", "preset authoring and scoped Agent composition", "cmd/sta/native_agent_presets.go", "preset list/select/authoring tests"),
		required("ctx.commands", "human command registry separate from model tools", "cmd/sta/web_command.go", "command lifecycle/catalog tests"),
		...
		required("ctx.skills", "merged filesystem skill catalog and scoped invocation", "internal/skill; cmd/sta", "skill discovery/invocation tests"),
		required("ctx.agents", "owned live Agent handles and lineage registry", "internal/agent; cmd/sta/agent_runtime.go", "Agent publication/disposal tests"),
		required("ctx.agentDefaultModel", "shared persisted default model route", "cmd/sta/modelcatalog.go", "model selection/catalog tests"),
		...
		required("ctx.permissionPresets", "preset changes update sandbox and approval together", "internal/config; cmd/sta", "permission runtime tests"),
		codeRuntime,
